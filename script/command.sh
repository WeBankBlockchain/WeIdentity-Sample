...
if [[ "$1" == daemon ]];then
    ...
    java ${JAVA_OPTS} -cp conf/:lib/*:app/* com.webank.weid.demo.command.DemoCommand $1 >/dev/null 2>&1 &
    ...
    echo "--------- start $1 ----------"
    java ${JAVA_OPTS} -cp conf/:lib/*:app/* com.webank.weid.demo.command.DemoCommand $1
fi
